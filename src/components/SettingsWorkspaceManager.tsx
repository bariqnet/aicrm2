...
import { confirmAlert, getResponseError, showErrorAlert, showSuccessAlert } from "@/lib/sweet-alert";
...
function toMembershipItems(payload: unknown): MembershipItem[] {
  const mapped: Array<MembershipItem | null> = rowsFromPayload(payload).map((item) => {
      ...
      const user = userSource && typeof userSource === "object"
        ? {
            id: typeof (userSource as Record<string, unknown>).id === "string"
              ? (userSource as Record<string, unknown>).id as string
              : undefined,
            name: typeof (userSource as Record<string, unknown>).name === "string"
              ? (userSource as Record<string, unknown>).name as string
              : undefined,
            email: typeof (userSource as Record<string, unknown>).email === "string"
...
export function SettingsWorkspaceManager({ workspaceId }: SettingsWorkspaceManagerProps) {
  ...
  const filteredInvites = useMemo(() => {
    ...
    const scoped = invites.filter((invite) => !invite.workspaceId || invite.workspaceId === activeWorkspace.id);
    return scoped;
  ...
  const loadSettings = useCallback(async () => {
    ...
    try {
      const [stagesResponse, invitesResponse, membershipsResponse, workspacesResponse] = await Promise.all([
      ...
      await Promise.all([
        ...
        ensureResponse(workspacesResponse, tr("Could not load workspaces.", "تعذر تحميل مساحات العمل."))
      ]);

      const [stagesPayload, invitesPayload, membershipsPayload, workspacesPayload] = await Promise.all([
      ...
      const message = loadError instanceof Error ? loadError.message : tr("Unable to load settings.", "تعذر تحميل الإعدادات.");
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  async function saveStage(stage: StageItem) {
    ...
    try {
      ...
      const message = actionError instanceof Error ? actionError.message : tr("Could not update stage.", "تعذر تحديث المرحلة.");
      await showErrorAlert(tr("Save failed", "فشل الحفظ"), message);
    } finally {
      setBusyKey(null);
    }
  ...
  async function moveStage(index: number, direction: -1 | 1) {
    ...
    try {
      ...
      const message = actionError instanceof Error ? actionError.message : tr("Could not reorder stages.", "تعذر إعادة ترتيب المراحل.");
  ...
  async function removeStage(stage: StageItem) {
    ...
    try {
      ...
      const message = actionError instanceof Error ? actionError.message : tr("Could not delete stage.", "تعذر حذف المرحلة.");
  ...
  async function createInvite() {
    ...
    try {
      ...
      const message = actionError instanceof Error ? actionError.message : tr("Could not send invite.", "تعذر إرسال الدعوة.");
  ...
  async function revokeInvite(invite: InviteItem) {
    ...
    try {
      ...
      const message = actionError instanceof Error ? actionError.message : tr("Could not revoke invite.", "تعذر إلغاء الدعوة.");
    ...
    <div className="space-y-4">
      <section className="panel-soft flex flex-wrap items-center justify-between gap-3 p-4 text-sm">
        <div>
          ...
          <p className="mt-1 font-medium text-fg">{activeWorkspace?.name ?? tr("Unknown workspace", "مساحة عمل غير معروفة")}</p>
          <p className="text-xs text-mutedfg">{activeWorkspace?.slug ? `${activeWorkspace.slug}` : activeWorkspace?.id ?? "-"}</p>
        </div>
        <div className="flex items-center gap-2 text-xs text-mutedfg">
          <span>{tr("Stages", "المراحل")}: {stages.length}</span>
          <span>{tr("Members", "الأعضاء")}: {filteredMembers.length}</span>
          <span>{tr("Pending invites", "الدعوات المعلقة")}: {filteredInvites.length}</span>
          <button
            type="button"
            className="btn h-8"
            onClick={() => {
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <article className="panel p-4">
              ...
              <div key={stage.id} className="rounded-lg border border-border bg-surface2 p-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <button
                    ...
                    disabled={busyKey !== null}
                  >
                    {busyKey === `save-stage-${stage.id}` ? <LoaderCircle size={14} className="animate-spin" /> : null}
                    {tr("Save", "حفظ")}
                  </button>
                  <button
                    type="button"
                  ...
                  <button
                    ...
                    disabled={busyKey !== null}
                  >
                    {busyKey === `delete-stage-${stage.id}` ? <LoaderCircle size={14} className="animate-spin" /> : <Trash2 size={14} />}
                  </button>
                </div>
              </div>
            ))}
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            >
              {busyKey === "add-stage" ? <LoaderCircle size={14} className="animate-spin" /> : <Plus size={14} />}
              {tr("Add", "إضافة")}
            </button>
          </div>
        </article>
        ...
        <article className="panel p-4">
          ...
          <div className="mb-3 flex flex-wrap items-center gap-2">
            ...
            >
              {busyKey === "create-invite" ? <LoaderCircle size={14} className="animate-spin" /> : <Plus size={14} />}
              {tr("Send", "إرسال")}
            </button>
          </div>

          {filteredInvites.length === 0 ? (
            <p className="panel-dashed p-4 text-sm text-mutedfg">{tr("No pending invites.", "لا توجد دعوات معلقة.")}</p>
          ) : (
            <div className="space-y-2">
              {filteredInvites.map((invite) => (
                <div key={invite.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-border bg-surface2 p-2 text-sm">
                  ...
                  <button
                    ...
                    disabled={busyKey !== null}
                  >
                    {busyKey === `revoke-invite-${invite.id}` ? <LoaderCircle size={14} className="animate-spin" /> : <Trash2 size={14} />}
                    {tr("Revoke", "إلغاء")}
                  </button>
                </div>
              ))}
      ...
      <article className="panel p-4">
        <div className="mb-3">
          ...
          <p className="mt-1 text-sm text-mutedfg">{tr("Current users with workspace access.", "المستخدمون الحاليون الذين لديهم وصول لمساحة العمل.")}</p>
        </div>

        {filteredMembers.length === 0 ? (
          <p className="panel-dashed p-4 text-sm text-mutedfg">{tr("No members found.", "لا يوجد أعضاء.")}</p>
        ) : (
            ...
            <table className="min-w-[680px] w-full text-left text-sm">
              ...
              <tbody>
                {filteredMembers.map((member) => (
                  <tr key={member.id} className="border-b border-border last:border-b-0 hover:bg-muted/40">
