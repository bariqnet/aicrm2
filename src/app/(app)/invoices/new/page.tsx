...
export default function NewInvoicePage() {
  ...
  return (
    <main className="app-page">
      <header>
        <Link href="/invoices" className="text-sm text-mutedfg hover:text-fg">
          {`${getDirectionalArrowSymbol(language, "back")} ${tr("Back to invoices", "العودة إلى الفواتير")}`}
        </Link>
        <h1 className="page-title mt-2">{tr("New invoice", "فاتورة جديدة")}</h1>
        <p className="page-subtitle">
          {tr(
            "Create and link an invoice to a contact or company.",
            "أنشئ فاتورة واربطها بجهة اتصال أو شركة.",
          )}
        </p>
      </header>
